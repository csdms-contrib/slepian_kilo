...
%          r   The sub-surface to surface initial correlation coefficient
...
switch blurs
  % When it's zero there is no blurring, when it's one it's blurring with
  % a delta function, so either way, skip ahead
 case {0,1}
  ...
  [invT,detT]=Tros(k,th,params);
  
  % Then put it all together... and all we have to worry about is a NaN in
  % Lk which we take care off in LOGLIROS. Note that Lk should be real. 
  warning off MATLAB:log:logOfZero
  disp('IN PROGRESS Correct according to SCO as of 04/02/2014')
  xbit=[params.z2-35000]*k(:);
  Xk=hformos(S11,Hk,invT);
  Lk=-2*log(S11)-log(detT)-Xk+xbit;
